...
/// Updates metadata with the transaction hash as the new genesis hash for item asset type transactions
pub fn update_metadata_with_tx_hash(transaction: &mut Transaction) {
    if transaction.asset_type == AssetType::Item {
        transaction.genesis_hash = transaction.hash.clone();
    }
}